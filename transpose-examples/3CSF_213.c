...
int transpose(struct taco_tensor_t *A, struct taco_tensor_t *B)
{
    ...
    for (int32_t B0_pos = B0_pos_arr[0]; B0_pos < B0_pos_arr[1]; B0_pos++)
    {
        ...
        for (int32_t B1_pos = B1_pos_arr[B0_pos]; B1_pos < B1_pos_arr[B0_end]; B1_pos++)
        {
            ...
            for (int32_t B2_pos = B2_pos_arr[B1_pos]; B2_pos < B2_pos_arr[B1_end]; B2_pos++)
            {
                ...
                C_old[c_size].idx0 = jB;
                C_old[c_size].idx1 = iB;
                C_old[c_size].perm = B2_pos;
    ...
    while (C0_pos < c_size)
    {
        int32_t iC = (C_old[C0_pos].idx0);
        int32_t C0_end = C0_pos + 1;
        while ((C0_end < c_size) && (((C_old[C0_end].idx1)) == iC))
        {
            C0_end++;
        }
        ...
        while (C1_pos < C0_end)
        {
            ...
            int perm = C_old[C1_pos].perm;
            for (int32_t C2_pos = B2_pos_arr[perm]; C2_pos < B2_pos_arr[perm + 1]; C2_pos++)
            {
                int32_t kC = B2_idx_arr[C2_pos];
                ...
                A_val_arr[A2_pos] = B_val_arr[C2_pos];
